...
public:
    ...
    void Multiply(const double &vector[], double &result[]) const
    {
        for (int i = 0; i < FPF_DIMENSION; i++)
        {
            result[i] = 0.0;
...
private:
    ...
    void GenerateCouplingMatrix(const double P[])
    {
        ...
        for (int i = 0; i < FPF_DIMENSION; i++)
        {
            for (int j = 0; j < FPF_DIMENSION; j++)
            {
                // Symmetric Component S(P) - Resonance/Coherence
                // S[i][j] = 0.1 * MathCos(P[i] + P[j])
                double S_ij = 0.1 * MathCos(P[i] + P[j]);
                
                // Antisymmetric Component A(P) - Orbital Dynamics/Flow
                // A[i][j] = 0.2 * MathSin(P[i] - P[j])
                double A_ij = 0.2 * MathSin(P[i] - P[j]);
                
                // J[i][j] = S[i][j] + A[i][j]
                m_J.Set(i, j, S_ij + A_ij);
            }
        }
    }    for (int i = 0; i < FPF_DIMENSION; i++)
        {
...
public:
    ...
    void Initialize(double initial_P[FPF_DIMENSION])
    {
        for (int i = 0; i < FPF_DIMENSION; i++)
            m_P[i] = initial_P[i];
    }
    ...
    void GetState(double P_out[]) const
    {
        for (int i = 0; i < FPF_DIMENSION; i++)
            P_out[i] = m_P[i];
    }
